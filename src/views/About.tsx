  ...
  return <>
    ...
    {(!helpId || helpId === 'fc') && <div className={`home-box rel ${cn.greyer}`}>
      ...
      <ul>
        ...
        <li>Any Fleet Carrier can be updated from their edit panel, look for the <Icon className="btn icon-inline" iconName='Refresh' style={{ color: appTheme.palette.themePrimary }} /> button top/left. Note: this relies on the FC owner having logged in within the past 30 days. If the owner plays through Epic, it may be necessary for them to be actively playing the game.</li>
        <li>Fleet Carrier purchase and market sell orders can also be viewed from the edit panel, look for the <Icon className="btn icon-inline" iconName='Shop' style={{ color: appTheme.palette.themePrimary }} /> button.</li>
        <li>In cargo grids (if a Fleet Carrier has been refreshed recently) each FC commodity grid cell will show a graphic, <Icon className="btn icon-inline" iconName='CaretRight8' style={{ color: appTheme.palette.blue, background: appTheme.palette.blueDark, padding: '3px 1px' }} /> for market purchase or <Icon className="btn icon-inline" iconName='CaretLeft8' style={{ color: appTheme.palette.green, background: appTheme.palette.greenDark, padding: '3px 1px' }} /> for sell orders. These  can be clicked to see the price and when orders were last refreshed.</li>
        <IconBtnScrollTop />
      </ul>
    </div>}
    ...
    {(!helpId || helpId === 'nexus') && <div className={`home-box rel ${cn.greyer}`}>
      ...
      <ul>
        ...
        <li>Use the <Icon className="btn icon-inline" iconName='MSNVideosSolid' style={{ color: appTheme.palette.themePrimary }} /> buttons on each system to see what specific cargo is needed and which linked FCs have the necessary supplies. The top most <Icon className="btn icon-inline" iconName='MSNVideosSolid' style={{ color: appTheme.palette.black }} /> button at the top shows the totals needed for the entire nexus.</li>
        <li>A nexus assumes you'll finish building in each system before moving onto the next, but it's okay if you skip ahead. </li>
